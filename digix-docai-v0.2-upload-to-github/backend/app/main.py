...
app=FastAPI(title="DigiX DocAI API",version="0.3.1")
app.add_middleware(CORSMiddleware,allow_origins=[x.strip() for x in settings.cors_origins.split(",")],allow_credentials=True,allow_methods=["*"],allow_headers=["*"])
ALLOWED={"application/pdf","image/png","image/jpeg"};DOCUMENTS={}
@app.get("/health")
def health():return {"status":"ok","service":"digix-docai","version":"0.3.1"}
...
async def analyze_document(file:UploadFile=File(...)):
 ...
 try:
  text=extract_text(file.filename or "document",file.content_type or "",data);r=analyze(file.filename or "document",file.content_type or "",data,text_override=text);did=str(uuid4());r.document_id=did;DOCUMENTS[did]={"text":text,"fields":r.extracted_fields};return r
