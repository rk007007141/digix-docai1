...
app=FastAPI(title="DigiX DocAI API",version="0.4.0")
app.add_middleware(CORSMiddleware,allow_origins=[x.strip() for x in settings.cors_origins.split(",")],allow_credentials=True,allow_methods=["*"],allow_headers=["*"])
ALLOWED={"application/pdf","image/png","image/jpeg"};DOCUMENTS={}
@app.get("/health")
def health():return {"status":"ok","service":"digix-docai","version":"0.4.0"}
...
async def analyze_document(file:UploadFile=File(...)):
 ...
 try:
  text,words=extract_document(file.filename or "document",file.content_type or "",data)
  r=analyze(file.filename or "document",file.content_type or "",data,text_override=text,words=words);did=str(uuid4());r.document_id=did
  DOCUMENTS[did]={"text":text,"words":words,"fields":r.extracted_fields};return r
 except Exception as e:print("PROCESSING ERROR:",repr(e));raise HTTPException(422,"The document could not be processed.")
...
def ask_document(req:QuestionRequest):
 ...
 a,c,g,e=answer_question(req.question,doc["text"],doc["fields"]);return QuestionAnswer(answer=a,confidence=c,grounded=g,evidence=e)
