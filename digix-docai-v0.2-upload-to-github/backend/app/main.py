...
app=FastAPI(title="DigiX DocAI API",version="0.2.2")
app.add_middleware(CORSMiddleware,allow_origins=[x.strip() for x in settings.cors_origins.split(",")],allow_credentials=True,allow_methods=["*"],allow_headers=["*"])
ALLOWED={"application/pdf","image/png","image/jpeg"}

@app.get("/health")
def health():return {"status":"ok","service":"digix-docai","version":"0.2.2"}
...
async def analyze_document(file:UploadFile=File(...)):
    ...
    try:return analyze(file.filename or "document",file.content_type or "",data)
    except Exception:
        raise HTTPException(422,"The document could not be processed. Please try a clearer PDF, JPG or PNG file.")
